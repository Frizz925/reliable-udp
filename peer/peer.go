...
func (p *Peer) sendMessage(addr string) error {
	...
	buf := make([]byte, 65535)
	for {
		if _, err := conn.Write([]byte(p.message)); err != nil {
			return err
		}
		log.Infof("Sent message to peer at %s: %s", addr, p.message)
		n, err := conn.Read(buf)
		if err != nil {
			return err
		}
		message := string(buf[:n])
		log.Infof("Received message from peer at %s: %s", addr, message)
...
func (p *Peer) handlePeer(conn net.Conn) error {
	...
	for {
		...
		message := string(buf[:n])
		log.Infof("Received message from peer at %s: %s", conn.RemoteAddr(), message)
		if _, err := conn.Write([]byte(p.message)); err != nil {
			return err
		}
		log.Infof("Sent message to peer at %s: %s", conn.RemoteAddr(), p.message)
		time.Sleep(LoopInterval)
	}
}
