...
func New(laddr *net.UDPAddr, priv protocol.PrivateKey) (*Mux, error) {
	...
	m.wg.Add(4)
	go m.routineDemux()
	go m.routineDeserialize()
	go m.routineSerialize()
	go m.routineMux()

	m.wg.Add(2)
	go m.routineWrite(conn)
	go m.routineRead(conn)

	log.Debugf("Created new mux: %+v", m)
	m.closed.Set(false)
	return m, nil
}
...
func (m *Mux) routineRead(conn *net.UDPConn) {
	defer m.wg.Done()
...
func (m *Mux) routineSerialize() {
	...
	for {
		...
		case out := <-m.queue.serialize:
			packet := out.Packet
			log.Debugf("Sending packet: %+v", packet)
...
func (m *Mux) routineWrite(conn *net.UDPConn) {
	defer m.wg.Done()
	for {
		select {
